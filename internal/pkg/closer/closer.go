...
// Closer is a helper to close multiple resources.
// It is safe to call Add concurrently.
// Close will call all added closers in reverse order.
// If any of the closers returns an error, it will be returned by Close.
// Close will return immediately if called more than once.
type Closer struct {
	closers  []closer
	mu       sync.Mutex
	isDone   chan struct{}
